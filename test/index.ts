...
import { readable, writable, derived, get } from "../src/index";
...
describe("store", () => {
	describe("writable", () => {
		...
		it("call conditioned, only if the condition is met", () => {
			const obj = {};
			let called = 0;

			const store = writable(obj);

			store.conditioned({ call: true }, () => {
				called += 1;
			});
		...
		it("calls provided conditioned handler", () => {
			let called = 0;
			...
			const unsubscribe1 = store.conditioned(1, () => {});
			assert.equal(called, 1);

			const unsubscribe2 = store.conditioned(2, () => {});
			assert.equal(called, 1);
